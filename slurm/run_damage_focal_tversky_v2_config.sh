...
# One-row runner for the damage_focal_tversky_v2 campaign.
# Time limits are supplied by sbatch from configs/damage_focal_tversky_v2.csv.
...
CONFIG_CSV="${CONFIG_CSV:-configs/damage_focal_tversky_v2.csv}"
SUMMARY_CSV="outputs/predictions/damage_focal_tversky_v2_summary.csv"
FORCE="${FORCE:-0}"
...
required_vars=(
  ...
  SAMPLER DAMAGE_SAMPLING_ALPHA HIGH_DAMAGE_THRESHOLD BASE_CHANNELS SEED NUM_WORKERS
)
...
mkdir -p outputs/checkpoints outputs/predictions/damage_focal_tversky_v2
...
METRICS_JSON="outputs/predictions/damage_focal_tversky_v2/${EXPERIMENT}_test_metrics.json"
METRICS_CSV="outputs/predictions/damage_focal_tversky_v2/${EXPERIMENT}_test_metrics.csv"
...
rebuild_summary() {
  python scripts/rebuild_damage_focal_tversky_v2_summary.py \
    --config "$CONFIG_CSV" \
    --output "$SUMMARY_CSV"
}
...
run_job() {
  ...
  echo "damage_focal_tversky_v2 job"
