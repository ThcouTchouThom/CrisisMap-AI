#!/usr/bin/env bash

set -euo pipefail
cd "${HOME}/work/CrisisMap-AI"

CONFIG_CSV="${CONFIG_CSV:-configs/building100_sweep_v1_relaunch.csv}"
RESUME_INCOMPLETE="${RESUME_INCOMPLETE:-1}"
FORCE_INCOMPLETE="${FORCE_INCOMPLETE:-0}"

if [ ! -f "$CONFIG_CSV" ]; then
  echo "Missing config CSV: $CONFIG_CSV" >&2
  exit 1
fi

echo "Submitting Building100 missing/stale SMP rows only."
echo "Skipping model=unet rows so the 3 completed local U-Net jobs are not retrained."
echo "RESUME_INCOMPLETE=${RESUME_INCOMPLETE} FORCE_INCOMPLETE=${FORCE_INCOMPLETE}"
echo "Config: $CONFIG_CSV"
echo

tail -n +2 "$CONFIG_CSV" | while IFS=, read -r \
  experiment model train_csv val_csv test_csv input_mode loss augment_mode sampler \
  sampler_alpha lr image_size batch_size epochs time_limit num_workers
do
  [ -n "$experiment" ] || continue
  if [ "$model" = "unet" ]; then
    echo "Skipping completed local U-Net row: ${experiment}"
    continue
  fi

  job_name="b100_smp_${experiment:0:64}"
  export_arg="ALL,CONFIG_CSV=${CONFIG_CSV},EXPERIMENT=${experiment},MODEL=${model},TRAIN_CSV=${train_csv},VAL_CSV=${val_csv},TEST_CSV=${test_csv},INPUT_MODE=${input_mode},LOSS=${loss},AUGMENT_MODE=${augment_mode},SAMPLER=${sampler},SAMPLER_ALPHA=${sampler_alpha},LR=${lr},IMAGE_SIZE=${image_size},BATCH_SIZE=${batch_size},EPOCHS=${epochs},NUM_WORKERS=${num_workers},RESUME_INCOMPLETE=${RESUME_INCOMPLETE},FORCE_INCOMPLETE=${FORCE_INCOMPLETE}"
  result="$(
    sbatch \
      --time="$time_limit" \
      --job-name="$job_name" \
      --export="$export_arg" \
      slurm/run_building100_config.sh
  )"
  echo "${experiment} -> ${result}"
done

echo
echo "Submitted SMP-only Building100 relaunch rows. Use email notifications and logs instead of frequent scheduler polling."
