#!/usr/bin/env bash
#SBATCH --job-name=damage_extra
#SBATCH --account=def-zonata_gpu
#SBATCH --partition=gpubase_bygpu_b2
#SBATCH --gres=gpu:h100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=96G
#SBATCH --time=04:00:00
#SBATCH --output=/home/tgrjlt2/scratch/CrisisMap-AI/logs/%x-%j.out
#SBATCH --error=/home/tgrjlt2/scratch/CrisisMap-AI/logs/%x-%j.err
#SBATCH --mail-user=t.gourjault@gmail.com
#SBATCH --mail-type=BEGIN,END,FAIL,TIME_LIMIT

# Email notifications to avoid frequent scheduler polling.
# Generic one-row runner for the extra damage sweep. Time limit and job name
# are normally overridden by slurm/submit_damage_extra_sweep_v1.sh from the CSV.

set -euo pipefail
cd "${HOME}/work/CrisisMap-AI"
export PYTHONPATH="${PWD}/src:${PWD}:${PYTHONPATH:-}"

module --force purge
module load StdEnv/2023
module load python/3.11
module load gcc
module load arrow/23.0.1
module load cuda
module load opencv/4.13.0

source "${HOME}/virtualenvs/crisismap-ai/bin/activate"
export TRITON_CACHE_DIR="${HOME}/scratch/CrisisMap-AI/triton_cache"
mkdir -p "${TRITON_CACHE_DIR}" "${HOME}/scratch/CrisisMap-AI/run_logs"
mkdir -p outputs/checkpoints outputs/predictions

ROOT="data/raw/xbd/train"
CONFIG_CSV="${CONFIG_CSV:-configs/damage_extra_sweep_v1.csv}"
SUMMARY_CSV="outputs/predictions/unet_1024_damage_extra_sweep_v1_summary.csv"
FORCE="${FORCE:-0}"
FORCE_INCOMPLETE="${FORCE_INCOMPLETE:-0}"
RESUME_INCOMPLETE="${RESUME_INCOMPLETE:-0}"
JOB_ID="${SLURM_JOB_ID:-manual}"

required_vars=(
  EXPERIMENT SPLIT TRAIN_CSV VAL_CSV TEST_CSV IMAGE_SIZE BATCH_SIZE LOSS
  CLASS_WEIGHTS LR EPOCHS AUGMENT_MODE SAMPLER DAMAGE_SAMPLING_ALPHA
  AUGMENT_PROB DAMAGE_AUGMENT_THRESHOLD HIGH_DAMAGE_THRESHOLD NUM_WORKERS
)
for name in "${required_vars[@]}"; do
  if [ -z "${!name:-}" ]; then
    echo "Missing required environment variable: ${name}" >&2
    exit 1
  fi
done

CLASS_WEIGHTS="${CLASS_WEIGHTS//;/ }"
OUTPUT_DIR="outputs/checkpoints/${EXPERIMENT}"
HISTORY_JSON="${OUTPUT_DIR}/metrics_history.json"
BEST_CHECKPOINT="${OUTPUT_DIR}/best_unet.pt"
LAST_CHECKPOINT="${OUTPUT_DIR}/last_unet.pt"
METRICS_JSON="outputs/predictions/${EXPERIMENT}_test_metrics.json"
RUN_LOG="${HOME}/scratch/CrisisMap-AI/run_logs/${EXPERIMENT}-${JOB_ID}.log"

history_complete() {
  local history_json="$1"
  local expected_epochs="$2"
  [ -f "$history_json" ] || return 1
  python - "$history_json" "$expected_epochs" <<'PY'
import json
import sys
from pathlib import Path

path = Path(sys.argv[1])
expected = int(sys.argv[2])
try:
    data = json.loads(path.read_text(encoding="utf-8"))
except Exception:
    sys.exit(1)
history = data if isinstance(data, list) else data.get("history") if isinstance(data, dict) else None
if not isinstance(history, list) or len(history) < expected:
    sys.exit(1)
epochs = []
for item in history:
    if isinstance(item, dict) and "epoch" in item:
        try:
            epochs.append(int(item["epoch"]))
        except (TypeError, ValueError):
            pass
if epochs and max(epochs) < expected:
    sys.exit(1)
sys.exit(0)
PY
}

rebuild_summary() {
  python scripts/rebuild_damage_extra_summary.py \
    --config "$CONFIG_CSV" \
    --output "$SUMMARY_CSV"
}

ensure_inputs() {
  local missing=0
  for path in "$TRAIN_CSV" "$VAL_CSV" "$TEST_CSV"; do
    if [ ! -f "$path" ]; then
      echo "Missing split CSV: $path" >&2
      missing=1
    fi
  done
  if [ "$missing" = "1" ]; then
    exit 1
  fi
}

run_job() {
  local weights_array
  read -r -a weights_array <<< "$CLASS_WEIGHTS"

  echo "Extra damage sweep job"
  echo "Experiment: ${EXPERIMENT}"
  echo "Split=${SPLIT}"
  echo "Train=${TRAIN_CSV}"
  echo "Val=${VAL_CSV}"
  echo "Test=${TEST_CSV}"
  echo "IMAGE_SIZE=${IMAGE_SIZE} BATCH_SIZE=${BATCH_SIZE} EPOCHS=${EPOCHS}"
  echo "LOSS=${LOSS} CLASS_WEIGHTS=${CLASS_WEIGHTS} LR=${LR}"
  echo "AUGMENT_MODE=${AUGMENT_MODE} SAMPLER=${SAMPLER} DAMAGE_SAMPLING_ALPHA=${DAMAGE_SAMPLING_ALPHA}"
  echo "FORCE=${FORCE} FORCE_INCOMPLETE=${FORCE_INCOMPLETE} RESUME_INCOMPLETE=${RESUME_INCOMPLETE}"

  if [ "$FORCE" = "1" ]; then
    echo "FORCE=1: removing prior outputs for this experiment."
    rm -rf -- "$OUTPUT_DIR"
    rm -f -- "$METRICS_JSON"
  elif history_complete "$HISTORY_JSON" "$EPOCHS" && [ -f "$METRICS_JSON" ]; then
    echo "Complete run and metrics found; skipping."
    rebuild_summary
    return 0
  elif [ -d "$OUTPUT_DIR" ]; then
    if history_complete "$HISTORY_JSON" "$EPOCHS" && [ -f "$BEST_CHECKPOINT" ]; then
      echo "Training complete; evaluating missing metrics."
    elif [ "$RESUME_INCOMPLETE" = "1" ] && [ -f "$LAST_CHECKPOINT" ]; then
      echo "RESUME_INCOMPLETE=1: resuming from last_unet.pt."
      rm -f -- "$METRICS_JSON"
    elif [ "$FORCE_INCOMPLETE" = "1" ]; then
      echo "FORCE_INCOMPLETE=1: removing incomplete output before retraining."
      rm -rf -- "$OUTPUT_DIR"
      rm -f -- "$METRICS_JSON"
    else
      echo "WARNING: Incomplete output folder; not evaluating partial checkpoint: $OUTPUT_DIR"
      echo "WARNING: Relaunch with FORCE_INCOMPLETE=1 to retrain only this incomplete run."
      rebuild_summary
      return 1
    fi
  fi

  if ! history_complete "$HISTORY_JSON" "$EPOCHS"; then
    resume_args=()
    if [ "$RESUME_INCOMPLETE" = "1" ] && [ -f "$LAST_CHECKPOINT" ]; then
      resume_args=(--resume-checkpoint "$LAST_CHECKPOINT")
    fi

    python -u -m src.crisismap.training.train_unet \
      --root "$ROOT" \
      --train-csv "$TRAIN_CSV" \
      --val-csv "$VAL_CSV" \
      --output-dir "$OUTPUT_DIR" \
      --image-size "$IMAGE_SIZE" \
      --batch-size "$BATCH_SIZE" \
      --epochs "$EPOCHS" \
      --target-mode 3-class \
      --loss "$LOSS" \
      --class-weights "${weights_array[@]}" \
      --lr "$LR" \
      --num-workers "$NUM_WORKERS" \
      --augment-mode "$AUGMENT_MODE" \
      --augment-prob "$AUGMENT_PROB" \
      --damage-augment-threshold "$DAMAGE_AUGMENT_THRESHOLD" \
      --sampler "$SAMPLER" \
      --damage-sampling-alpha "$DAMAGE_SAMPLING_ALPHA" \
      --high-damage-threshold "$HIGH_DAMAGE_THRESHOLD" \
      "${resume_args[@]}"
  fi

  if ! history_complete "$HISTORY_JSON" "$EPOCHS" || [ ! -f "$BEST_CHECKPOINT" ]; then
    echo "ERROR: Training is not complete; refusing to evaluate partial checkpoint." >&2
    exit 1
  fi

  if [ ! -f "$METRICS_JSON" ]; then
    python -u -m src.crisismap.evaluation.evaluate_unet \
      --root "$ROOT" \
      --split-csv "$TEST_CSV" \
      --checkpoint "$BEST_CHECKPOINT" \
      --output "$METRICS_JSON" \
      --image-size "$IMAGE_SIZE" \
      --batch-size 1 \
      --target-mode 3-class \
      --num-workers "$NUM_WORKERS"
  fi

  rebuild_summary
}

ensure_inputs
run_job 2>&1 | tee "$RUN_LOG"
