...
def load_checkpoint(path: Path, map_location: torch.device) -> object:
    """Load project checkpoints with PyTorch >=2.6 compatibility.

    These checkpoints are generated by this project and may contain metadata such
    as pathlib.Path objects. They are trusted local artifacts, so weights_only is
    explicitly disabled when the PyTorch version supports it.
    """
